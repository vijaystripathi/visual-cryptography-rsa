...
import com.user.util.Image_Merge;
...
public class ColorEncryptImage extends HttpServlet
{
	@Override
	protected void service(HttpServletRequest request, HttpServletResponse response)throws ServletException, IOException 
	{
		...
		try 
		{
			...
			if(flag)
			{
				...
				outFilePath = request.getRealPath("")+"\\Files\\VCShares\\"+"Share1_"+fileName;
				
				String outFilePath1 = request.getRealPath("")+"\\Files\\VCShares\\"+"Share2_"+fileName;
				
				System.out.println("inFilePath "+inFilePath);
				
				String fileExtension=fileName.substring(fileName.lastIndexOf(".")+1,fileName.lastIndexOf(""));//bmp
	 		    fileExtension = "."+fileExtension;//.bmp
	 		    
				// Getting The Random Share1
				//outFileLocation = root+"\\Files\\RandomShare\\";
				String outFileName = "Share1_"+fileName;
				
				//img = ImageProcess.readImage(inFilePath);
				
				
				//Image_Merge.mergeImage(img,outFilePath,outFileName);
				...
				//BufferedImage img=ImageProcess.readImage(filename);
				//ArrayList<String> list1 = ImageProcess.processImage(img, 2, "Share", fileName,outFilePath);
				 //out1=list1.get(0);
				// out2=list1.get(1);
				...
				ArrayList<String> list1 = ImageProcess.processImage(img1, 2, "Share", fileName,outFilePath,outFilePath1);
				...
				rd=request.getRequestDispatcher("/JSP/User/display_vc_color_img.jsp?no=1");
			...
			if(submi.trim().equals("Encrypt Share1"))
			{
				...
				else
				{
					...
					if(flg)
					{
						...
						rd=request.getRequestDispatcher("/JSP/User/color_keygenerate.jsp?no=1");
						rd.forward(request, response);
					}
					else
					{
						...
						request.setAttribute("share1",share1);
						request.setAttribute("share2",share2);
						rd=request.getRequestDispatcher("/JSP/User/color_keygenerate.jsp?no=2");
						rd.forward(request, response);
			...
			if(submi.trim().equals("Encrypt Share2"))
			{
				...
				else
				{
					...
					if(flg)
					{
						...
						request.setAttribute("share1",share1);
						request.setAttribute("share2",share2);
						rd=request.getRequestDispatcher("/JSP/User/color_keygenerate.jsp?no=3");
						rd.forward(request, response);
					}
					else
					{
						...
						request.setAttribute("share1",share1);
						request.setAttribute("share2",share2);
						rd=request.getRequestDispatcher("/JSP/User/color_keygenerate.jsp?no=4");
						rd.forward(request, response);
				...
				System.out.println("File Name : " + fileName);
					...
					//String srcFilePath = "rose.jpeg";
					 ...
					 response.sendRedirect(request.getContextPath()+"/DownloadFile?fileNames="+destFilePath+"&download=true");
						...
						request.setAttribute("share1",share1);
						request.setAttribute("share2",share2);
						rd=request.getRequestDispatcher("/JSP/User/color_keygenerate.jsp?no=5");
						rd.forward(request, response);
