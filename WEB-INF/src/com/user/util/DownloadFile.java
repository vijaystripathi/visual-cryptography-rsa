		...
		try 
		{
			...
			if(filePath != null && filePath.trim().length()>0)
			{
				...
				if(download != null && download.trim().length()>0)
				{
					...
					if (contenttype != null)
					{
						//System.out.println("Hi f fghfh hjh");
						String responseType = getResponseContentType(contenttype);
						response.setContentType(responseType);
					}
					else 
					{
						//System.out.println("Hi f fghfh hlpa");
						response.setContentType("application/octet-stream");
					}
				...
				if(download != null && download.trim().length()>0)
				{
					System.out.println("Hi file download");
					response.setHeader("Content-Disposition","attachment; filename=\""+ prefile.getName()+"\"");
					//response.sendRedirect(request.getContextPath()+"/JSP/User/keygenerate.jsp?no=1"+"&fileName="+fnam);
